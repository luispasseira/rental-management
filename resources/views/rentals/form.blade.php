...
<div class="form-group">
    ...
    <div>
        <input id="datepicker1" type="text" name="start_date" class="form-control" placeholder="Dia-Mês-Ano"
               value="@if(date('d-m-Y', strtotime($rental->end_date)) == '01-01-1970') {{ $now->format('d-m-Y') }} @else {{ date('d-m-Y', strtotime($rental->start_date)) }} @endif"/>
    </div>
</div>
<div class=" form-group">
    ...
    <div>
        <input id="datepicker2" type="text" name="end_date" class="form-control" placeholder="Dia-Mês-Ano"
               value="@if(date('d-m-Y', strtotime($rental->end_date)) != '01-01-1970') {{ date('d-m-Y', strtotime($rental->end_date)) }} @endif"/>
    </div>
</div>
    ...
    <div class="form-group">
        <select class="form-control" name="vehicle_id">
            ...
            @else
                ...
                @foreach($vehicles as $vehi)
                    <option name="vehicle"
                            value={{$vehi->id}} @if ($vehi->id == $vehicle->id) {{ 'selected' }} @endif>
                        {{ $vehi->registration }}
                    </option>
                @endforeach
            @endif
